...
#         self.provider = settings.llm_provider.lower()
#         self.gemini_key = settings.gemini_api_key
#         self.gemini_endpoint = settings.gemini_endpoint
#         self.openai_key = settings.openai_api_key
#         self.openai_endpoint = "https://api.openai.com/v1/chat/completions"
#     async def analyze_multimodal(
#         self,
...
#     ) -> Dict[str, Any]:
#         """Analyze multimodal input: image, text, and spoken instructions."""
#         # Convert image to short-lived URL
#         image_url = None
#         if image_base64:
#             image_url = await base64_to_temp_url(image_base64)

#         # Combine user intent and context
#         user_instruction = (
#             f"User instruction (from voice or text):\n{transcription or text or '(none)'}\n\n"
#             "If the instruction is unclear, intelligently infer what the user likely wants "
#             "based on the image and text content. Respond helpfully and concisely."
#         )

#         # Construct multimodal prompt
#         prompt = (
#             "You are a multimodal assistant. Analyze the image and any provided instructions.\n"
#             "Perform the requested operation—describe, extract data, summarize, or critique.\n"
#             "Return your response in clear **Markdown** format, using proper headers, bullet points, and code blocks where relevant.\n"
#             "The output will be exported to Linear.app or Notion, so ensure it’s clean, organized, and professional.\n\n"
#             f"{user_instruction}\n\n"
#             f"Image reference: {image_url or '(no image provided)'}"
#         )


#         # --- Gemini ---
#         if self.provider == "gemini":
#             # headers = {"Content-Type": "application/json", "Authorization": f"Bearer {self.gemini_key}"}
...
#                     "parts": (
#                         [{"text": prompt}] + 
#                         ([{"inline_data": {"mime_type": "image/jpeg", "data": image_base64}}] if image_base64 else [])
#                     )
#                 }]
#             }

#             async with httpx.AsyncClient(timeout=60.0) as client:
#                 resp = await client.post(self.gemini_endpoint, params=params, json=body, headers=headers)
...
#             else:
#                 text_out = json.dumps(jr)

#         # --- OpenAI ---
#         elif self.provider == "openai":
#             headers = {"Authorization": f"Bearer {self.openai_key}"}
#             messages = [
#                 {"role": "system", "content": "You are a multimodal AI assistant that interprets images, text, and speech together."},
#                 {"role": "user", "content": [{"type": "text", "text": prompt}]},
#             ]
...
#                 "type": "image_url",
#                 "image_url": {
#                     "url": image_url or f"data:image/jpeg;base64,{image_base64}"
#                 },
#             })
...
#             text_out = jr["choices"][0]["message"]["content"]
