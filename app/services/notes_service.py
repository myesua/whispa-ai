...
class NotesService:
    ...
    async def process_final_notes(self, markdown_text: str, voice_text: Optional[str], user_id: Optional[str], privacy_mode: bool, session_id: str):
        """Processes the final markdown text for saving and metadata."""
        title, body = extract_title_and_body(markdown_text)
        
        saved = None
        if user_id and not privacy_mode:
            saved = await persist_note_if_allowed(
                user_id=user_id, voice_text=voice_text, title=title, body=body, privacy_mode=privacy_mode
            )
